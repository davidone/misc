...
class MainTescoAddon(TestCase):
    ...
    def test_send_po(self, mock_http_client):
        mock_http_client.return_value.getresponse.return_value = mock.MagicMock(status=200)
        ret_val = wrap.send_po("foobar")
        self.assertEqual(ret_val, True)

        mock_http_client.return_value.getresponse.return_value = mock.MagicMock(status=403)
        ret_val = wrap.send_po("foobar")
        self.assertEqual(ret_val, False)

    @mock.patch('argparse.ArgumentParser.parse_args', return_value=argparse.Namespace(days="2020-07-02", dtb_path="/"))
    ...
    def test_main(self, mock_argparse, mock_define_po_keys, mock_check_tesco, mock_process_tesco, mock_send_po, mock_time_sleep):
